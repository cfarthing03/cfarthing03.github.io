(()=>{
  ...
  async function spillScrolls(button, section){
    ...
    cards.forEach((card,index)=>card.style.transitionDelay=`${Math.min(index,10)*.045}s`);
    await sleep(90);
  ...
  document.querySelectorAll('.book').forEach(book=>{
    ...
    book.addEventListener('focusout',e=>{if(!book.contains(e.relatedTarget))book.classList.remove('open')});
  });
})();